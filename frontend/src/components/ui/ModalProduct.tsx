...
export default function ModalProduct({
    ...
    const onSubmit = handleSubmit((data) => {
        ...
        if(!product.id) {
            ...
            axios.patch(`http://localhost:8080/api/hardwarestore/product/${product.id}`, formData)
                .then(() => {
                        ...
                        <form onSubmit={onSubmit} className="w-full bg-white shadow-md p-6">
                            <div className="flex flex-wrap -mx-3 mb-6">
                                <div className="w-full md:w-full px-3 mb-6">
                                    ...
                                    <input {...register("title", {required: true, })} className="appearance-none block w-full bg-white text-gray-900 font-medium border border-gray-400 rounded-lg py-3 px-3 leading-tight focus:outline-none focus:border-[#98c01d]" type="text" name="title" placeholder="Title" />
                                </div>
                                <div className="w-full px-3 mb-6">
                                    ...
                                    <textarea {...register("description", { required: true, })} rows={4} className="appearance-none block w-full bg-white text-gray-900 font-medium border border-gray-400 rounded-lg py-3 px-3 leading-tight focus:outline-none focus:border-[#98c01d]" placeholder="Description" name="description"  ></textarea>
                                </div>
                                <div className="w-full md:w-full px-3 mb-6">
                                    <label className="block uppercase tracking-wide text-gray-700 text-sm font-bold mb-2" htmlFor="category_name">Category</label>
                                    <input {...register("category", { required: true, })} className="appearance-none block w-full bg-white text-gray-900 font-medium border border-gray-400 rounded-lg py-3 px-3 leading-tight focus:outline-none focus:border-[#98c01d]" type="text" name="category" placeholder="Category" />
                                </div>
                                <div className="w-full md:w-full px-3 mb-6">
                                    <label className="block uppercase tracking-wide text-gray-700 text-sm font-bold mb-2" htmlFor="category_name">Price</label>
                                    <input {...register("price", { required: true, })} className="appearance-none block w-full bg-white text-gray-900 font-medium border border-gray-400 rounded-lg py-3 px-3 leading-tight focus:outline-none focus:border-[#98c01d]" type="text" name="price" placeholder="Price" />
                                </div>
                                ...
                                <div className="w-full px-3 mb-8">
                                    <label className="mx-auto cursor-pointer flex w-full max-w-lg flex-col items-center justify-center rounded-xl border-2 border-dashed border-green-400 bg-white p-6 text-center" htmlFor="dropzone-file">
                                        ...
                                        <input id="dropzone-file" {...register("image", { required: true, })} type="file" className="hidden" name="image" accept="image/png, image/jpeg, image/webp" />
                                    </label>
                                </div>
